...
At the time of writing Vue is being re-written from scratch in Typescript (3.x) and the Class way of writing a component will become the defacto. (from a personal view this is a releaf as it seems so much cleaner than the current default)
...
| | VueJs | Aurelia | comments |
|-|-------|---------|----------|
| text | ```{{ item.name }}``` | ```${ item.name }``` | Vue is similar to handlebars, Aurelia uses the JS syntax |
| dynamic style | ```:style="attributeName"``` not sure how to do this on a calculated value | ```style="text-decoration:${ item.isComplete ? 'line-through' : ''}"``` | im sure there is a better way |
| dynamic class | ```:class="[ item.isComplete ? 'strike' : 'no-strike' ]"``` | ```class="${item.isComplete ? 'strike' : 'no-strike'}"``` | the better way :) |
| click function  | @click="addItem()" or v-bind:click="addItem()" | click.delegate="addItem()" | the ```@ ``` is shorthand for ```v-on:``` |
| repeater | ```<v-card v-for="(item, index) in items">``` | ```<md-card repeat.for="item of items">``` | aurelia provides a ```$index``` for all repeat.for |
| value binding | ```v-model="newItemContent"``` | ```value.bind="newItemContent"``` | note the .bind is similar to other parts i.e. ```show.bind``` and ```if.bind``` |
